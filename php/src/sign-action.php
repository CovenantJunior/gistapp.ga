			...
			if (mysqli_num_rows(mysqli_query($conn, $sql2)) >= 1) {
				...
				if ($insert == true) {
					...
					if ($create) {
						...
						$data = mysqli_query($conn, "INSERT INTO `".$fname."_".$lname."` (`frequency`, `user_id`, `image`, `alt`, `fname`, `lname`, `email`, `phone`, `password`, `about`, `LsMp`) VALUES ('', '".$user_id."', 'profile.png', 'profile-pic', '".$fname."', '".$lname."', '".$email."', '".$phone."', '".$password."', '".$about."', '".$date."');");
						if ($data) {
							...
							if ($create_list) {
								...
								if ($create_list1) {
									...
									$gId = password_hash($email, PASSWORD_DEFAULT);
									setcookie("GID", $gId, time() + 86400*90, "/");
...
<?php
	...
	if (isset($_POST['login'])&&($_POST['login']==true)) {
		...
		if (mysqli_num_rows($select) == false) {
			echo "Account doesn't exist. Please Create an Account";
		}
		else {
			echo mysqli_error($conn);
		}
