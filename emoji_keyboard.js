...
class EmojiKeyboard {
    ...
    click_on_emoji(kb, event) {
        if (!event.shiftKey) $('button#yactive-attach').show();$('button#inactive-attach').show();/*kb.toggle_window()*/
        const data = Object.assign({}, event.target.dataset)
        kb.callback(data, !event.shiftKey);
        setTimeout(function() {var message = $('textarea.text').val(); if(message==''){$('button#active-send').hide();$('label#active-attach').show();$('label#active-mic').show();} else {$('button#active-send').show();$('label#active-attach').hide();$('label#active-mic').hide();} }, 200);
        setTimeout(function() {var message = $('textarea.inactive-text').val(); if(message==''){$('button#inactive-send').hide();$('label#iactive-attach').hide();$('label#iactive-mic').hide();} else {$('button#inactive-send').show();$('label#inactive-attach').hide();$('label#inactive-mic').hide();} }, 200);
    }
    ...
    init_base_emojis() {
        ...
        fetch("emojis.json").then(async response => {
